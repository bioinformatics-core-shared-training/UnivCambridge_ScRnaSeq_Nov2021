...
p1 <- 
    ...
    tibble(UMI = log10(sce$sum)) %>%  
        ggplot(aes(x = "Sample", y = UMI)) +
            geom_violin()
        scale_y_log10() + 
        theme(plot.background = element_rect(fill = "transparent", colour = NA)) +
        ggtitle("log10(Total UMI count)") +
        xlab(NULL)
